# Lab 05 – Securing Your Network with Standard ACLs
...
3. **PROTECT-SW** – Restrict SSH access to the switch management plane to PCs and a designated TFTP server.
...
![Lab Topology](img/09-Topology.png)
...
When untrusted devices inject packets claiming to come from inside your PC subnet, they can bypass perimeter defenses and blend in with legitimate traffic. This practice, known as **IP spoofing**, enables attackers to:
...
Our **PROTECT-PC** ACL serves as a **spoof-protection** measure: it explicitly denies any packet whose **source IP** does not belong to the approved PC range (`198.18.U.128/26`). By applying this ACL **inbound** on the CORE router’s PC-facing interface, we ensure that:
...
Before you begin, create a spoof-address on **CORE** to test denial of unauthorized sources:

```shell
CORE(config)# interface Loopback130
CORE(config-if)# ip address 198.18.U.130 255.255.255.255
CORE(config-if)# exit
```
...
| ❌      | Spoof → PC            | `CORE# ping 198.18.U.129 source 198.18.U.130` | 0% success; packets dropped by PROTECT-PC                    |
...
CORE# ping 198.18.U.129 source 198.18.U.130     !-- FAIL
...
Plan where to apply the `PROTECT-SW` ACL.  Then answer the reflection questions to confirm your choices.
...
| SSH from spoof (CORE Loopback99) (denied) | `CORE# ssh admin@198.18.U.189` source `198.18.U.99` | ❌ Connection refused or timeout                                                      |
...
```bash
show ip access-lists PROTECT-SW 
show ip interface Vlan1 | include PROTECT-SW
